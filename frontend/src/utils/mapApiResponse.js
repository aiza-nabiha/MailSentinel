...
export function mapApiResponseToReportShape(api) {
  ...
  const edges = [...(primary.domain ? [{ a: "email", b: "domain", type: "verified", confidence: 1, reason: "Domain observed in this investigation" }] : []), ...matches.map((match) => ({ a: "email", b: match.email_id, type: "corroborated", confidence: 0.8, reason: array(match.signals).join(", ") || "Shared campaign infrastructure" }))];
  ...
  return {
    ...
    domain_info: primary.domain ? { domain: primary.domain, ip: primary.dns?.records?.A?.[0] || primary.ip || null, asn: primary.dns?.asn || primary.asn || null, hosting: primary.dns?.hosting || primary.hosting || null } : null,
    relay_path: received.map((hop, index) => ({ label: `Hop ${index + 1}`, sub: hop.from_host || hop.hostname || hop.from_ip || "Unknown host", verified: Boolean(hop.verified), trusted: Boolean(hop.trusted), ip: hop.from_ip || "—", location: hop.location || "Unknown", hosting: hop.hosting || "Unknown", note: hop.note || "Received header observation" })),
    campaign_correlation: api.campaign_correlation ? { matched_investigations: matches.length, nodes, edges } : null,
  };
}
